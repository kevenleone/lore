...
describe('a board in a nested collection', () => {
    // Nesting made a collection id a path, and the board surfaces printed that
    // path where a name belongs: `Work/Projects/Alpha` in a header, and the same
    // string truncated to `Work/Pro…` in the sidebar.
    const nested = [{ color: '#6b8cc4', id: 'Work/Projects/Alpha', name: 'Work/Projects/Alpha' }];

    function mountNested(taskView: 'board' | 'summary', boardId: null | string) {
        useStore.setState({
            boardId,
            boards: {},
            collections: nested,
            items: [task('t1', 'Cut the release branch', { collectionId: 'Work/Projects/Alpha' })],
            mainView: 'tasks',
            selectedId: null,
            taskView,
        });
    ...
    it('heads the board with the folder’s own name', () => {
        mountNested('board', 'Work/Projects/Alpha');

        expect(screen.getByText('Alpha')).toBeTruthy();
    });

    it('still says which Alpha, so two of them read apart', () => {
        mountNested('board', 'Work/Projects/Alpha');

        expect(screen.getByText('Work/Projects/')).toBeTruthy();
    });

    it('names it the same way on the overview card', () => {
        mountNested('board', null);
