...
describe('attachments from a URL', () => {
    beforeEach(openVault);

    const PNG = new Uint8Array([0x89, 0x50, 0x4e, 0x47, 1, 2, 3]);
    let server: null | ReturnType<typeof Bun.serve> = null;
    ...
    /** Stands in for the page the user pasted a URL from. */
    function origin(handler: (req: Request) => Response) {
        server = Bun.serve({ fetch: handler, port: 0 });
        return `http://127.0.0.1:${server.port}`;
    }

    const fromUrl = (url: string) => call('POST', '/attachments/from-url', { url });

    it('downloads an image and stores it like an upload', async () => {
        const base = origin(() => new Response(PNG, { headers: { 'content-type': 'image/png' } }));
        const res = await fromUrl(`${base}/photos/My%20Shot.png`);
        expect(res.status).toBe(201);
        expect(res.body.path).toBe('attachments/my-shot.png');

        const read = await app.handle(
            new Request(`http://127.0.0.1/${res.body.path}`, {
                headers: { authorization: `Bearer ${TOKEN}`, origin: 'tauri://localhost' },
            }),
        );
    ...
    it('names the file after the host when the URL has no basename', async () => {
        const base = origin(() => new Response(PNG, { headers: { 'content-type': 'image/jpeg' } }));
        expect((await fromUrl(base)).body.path).toBe('attachments/127-0-0-1.jpg');
    });

    it('refuses a URL that answers with a web page, not an image', async () => {
        const base = origin(
            () => new Response('<html></html>', { headers: { 'content-type': 'text/html' } }),
        );
        const res = await fromUrl(base);
        expect(res.status).toBe(422);
        expect(res.body).toEqual({ error: 'unsupported_type' });
    });

    it('reports a server that will not serve the file', async () => {
        const base = origin(() => new Response('nope', { status: 404 }));
        expect((await fromUrl(base)).body).toEqual({ error: 'fetch_failed' });
    });

    it('refuses a protocol that could read the disk', async () => {
        expect((await fromUrl('file:///etc/passwd')).body).toEqual({
            error: 'unsupported_protocol',
        });
    });

    it('refuses a data: URL', async () => {
        expect((await fromUrl('data:image/png;base64,iVBORw0KGgo=')).body).toEqual({
            error: 'unsupported_protocol',
        });
    });

    it('adds https:// to a bare host rather than refusing it', async () => {
        const base = origin(() => new Response(PNG, { headers: { 'content-type': 'image/png' } }));
        // Only the scheme check is under test; a bare host cannot reach the stub.
        expect((await fromUrl(base.replace('http://', ''))).body.error).toBe('fetch_failed');
    });
