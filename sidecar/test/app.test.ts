...
type Init = { origin?: null | string } & RequestInit;
...
describe('config', () => {
    it('uses the supplied token and an ephemeral port in production', () => {
        ...
        expect(c).toMatchObject({ dev: false, port: 0, token: 'abc' });
    });
    ...
    it('reads the vault and parent pid', () => {
        const c = loadConfig({ LORE_PARENT_PID: '42', LORE_TOKEN: 't', LORE_VAULT: '/tmp/v' });
